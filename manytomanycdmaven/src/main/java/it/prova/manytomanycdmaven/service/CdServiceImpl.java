...
//			cdTransientInstance.addToGeneri(genereTransientInstance);
	...
	@Override
	public List<Cd> cercaTuttiICdByGenere(Genere genereInstance) throws Exception {
		EntityManager entityManager = EntityManagerUtil.getEntityManager();
		...
		try {
			...
			// eseguo quello che realmente devo fare
			return cdDAO.findAllByGenere(genereInstance);
	...
	@Override
	public Cd cercaTramiteTitolo(String titoloCd) throws Exception {
		EntityManager entityManager = EntityManagerUtil.getEntityManager();

		try {
			cdDAO.setEntityManager(entityManager);

			return cdDAO.findByTitolo(titoloCd);
